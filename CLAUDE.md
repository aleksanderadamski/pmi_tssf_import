...
- **Accented names are not being stripped by this sync — investigated 2026-09-19, not reproduced.** Reported symptom: names carry Polish letters in ThoughtSpot but look transliterated (`Ą`→`A`, `Ł`→`L`) in Salesforce.
  ...
  **The load-bearing evidence is `verify_push.py`, not the diagnostic.** It compares the pushed manifest against Salesforce for `FirstName`/`LastName` across the **whole** population — 2,464 records — and reported mismatches in `Email` only. `diagnose_diacritics.py` then confirmed the same on a handful of accented members end to end (equal after NFC normalization; an NFC/NFD storage difference would read as equal, so this is "same characters", not "same bytes"). Take them in that order: a five-member sample must never override the full-population check.

  **The source appears to hold both spellings.** 182 of 2,467 currently-active members have a non-ASCII character in a name field; the rest are ASCII as ThoughtSpot supplies them. A human spot-check of the ThoughtSpot UI on 2026-09-19 found a common given name present both ways across two records (accented on one, plain on the other) — eyeballed, n=2, so treat it as corroboration rather than a measurement. The closure rests on the full-population `verify_push.py` run, not on that. Before debugging over an unaccented Contact, trace that member: `python diagnose_diacritics.py <PMI-id>`.

  **The standing check, and exactly what it covers:** `verify_push.py` compares `FirstName`/`LastName` for every pushed record, so a folded accent shows up as a mismatch. Its comparison is NFC-normalized, so a composed/decomposed byte difference is treated as the same text (and reported as INFO, since it still means something is re-encoding names) while a real fold — `ł` stored as `l` — is not canonically equivalent and fails. **It only covers the manifest → Salesforce leg.** The baseline is `output/pushed_<UTC>.csv`, written *after* the ThoughtSpot fetch, so if `/searchdata` ever began returning folded text the manifest and Salesforce would agree and it would pass forever. That leg is covered only by `diagnose_diacritics.py`.
