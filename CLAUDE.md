...
- **Accented names are not being stripped by this sync — investigated 2026-09-19, not reproduced.** Reported symptom: names carry Polish letters in ThoughtSpot but look transliterated (`Ą`→`A`, `Ł`→`L`) in Salesforce.
  ...
  **What was measured upstream:** 182 of 2,467 currently-active members have a non-ASCII character in a name field in ThoughtSpot. The remaining ~2,285 are ASCII *as the source supplies them*. That is all this shows — it does **not** establish that any of those 2,285 is a folded version of an accented name. So before debugging the sync over an unaccented Contact, look that member up in ThoughtSpot: `python diagnose_diacritics.py <PMI-id>` traces one member specifically, which is what actually settles a report about a named person.

  **Never "fix" this with a transliteration map or any other normalization** — the sync is reused by chapters in other countries, so every character check must be `ord(c) > 127`, never a per-language letter list. If accents ever *do* start disappearing, note that `Ł` (U+0141) has no Unicode decomposition, so NFKD leaves it alone while `Ą` NFKD-folds to `A`: seeing `Ł`→`L` means a real transliteration table (Unidecode, `iconv //TRANSLIT`, ICU, a cleansing package), not naive normalization.
