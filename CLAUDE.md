...
- **Sentinel dates are `-2208988800` (`1900-01-01`), and they are NOT "no expiry".** Measured 2026-09-18 by `report_sentinel_dates.py` over 10,495 live term rows: it is the *only* out-of-range value present, on **9 rows**, and those rows carry it in `Pmpstartdate`, `Pmpexpiredate` **and** `Pmporiginalgrantdate` simultaneously. A credential that never expires would still have a grant date, so "no expiry" is ruled out.
  ...
  **`_min_ignore_none()` picks the sentinel over a real date.** `-2208988800` is smaller than every plausible epoch, so in `dedupe_by_person()` the MIN merges — `Startdateforterm` (join date) and `Pmporiginalgrantdate` — let a sentinel row *beat* a real one, discarding the true value permanently. The MAX merges (`Enddateforterm`, `Pmpstartdate`, `Pmpexpiredate`) mask it correctly. Neither helper gates on `is_plausible_date_epoch()`, which this file's own rule requires of any raw-epoch comparison. It has not bitten yet only because the 9 sentinel rows belong to members the currency filter drops — **see NEXT_STEPS §13; don't assume it's harmless.**
