...
- **The sync never deletes, and never blanks a field.** Two guards in `salesforce_client.py`, both load-bearing because the integration user needs **Modify All** on Contact (to write the ~209 records it doesn't own) and that grant carries delete rights: (1) `WRITE_METHODS` pins the `/composite/sobjects` call to `PATCH`/`POST`, so no `DELETE` can be issued even by mistake; (2) a field that is `None`/`""` after its `FIELD_MAP` transform is **omitted from the payload**, not sent as `null` — a null would erase whatever the chapter entered by hand in the Salesforce UI. Consequence to accept: a value that legitimately disappears upstream (a lapsed cert) goes **stale** in Salesforce rather than being cleared. Clearing is a deliberate manual act. Don't "fix" this by writing nulls.
  ...
  **Measured 2026-09-18: 0 conflicts — but that is a snapshot, not a proof.** A one-off report (since removed, see *Retired tooling*) checked all ten mapped fields across the 1,088 currently-active members having at least one omitted field: every such field was already empty in Salesforce, so omitting changed nothing *then*. Two limits on that: only currently-active members were examined (a lapsed member's Contact still holds whatever the last sync wrote), and the risk this gotcha describes — a value present upstream now that disappears later — **cannot produce a conflict until after it happens**. So 0 meant "has not happened yet", not "cannot happen". Before relaxing this rule, re-measure rather than citing that number.
...
- **Accented names are not being stripped by this sync — investigated 2026-09-19, not reproduced.** Reported symptom: names carry Polish letters in ThoughtSpot but look transliterated (`Ą`→`A`, `Ł`→`L`) in Salesforce.

  **The load-bearing evidence is `verify_push.py`.** It compares the pushed manifest against Salesforce for `FirstName`/`LastName` across the **whole** population — 2,464 records — and reported mismatches in `Email` only. A one-off tracer (since removed) confirmed the same on a handful of accented members end to end.

  **The source holds both spellings.** 182 of 2,467 currently-active members have a non-ASCII character in a name field; the rest are ASCII as ThoughtSpot supplies them. A human spot-check of the ThoughtSpot UI on 2026-09-19 found a common given name present both ways across two records — eyeballed, n=2, corroboration rather than measurement, with the closure resting on the full-population run. So an unaccented Contact is PMI's spelling for that record.

  **The standing check, and exactly what it covers:** `verify_push.py` compares `FirstName`/`LastName` for every pushed record, so a folded accent shows up as a mismatch. Its comparison is NFC-normalized, so a composed/decomposed byte difference is treated as the same text (and reported as INFO, since it still means something is re-encoding names) while a real fold — `ł` stored as `l` — is not canonically equivalent and fails. It also prints how many accented values it checked, so a PASS distinguishes "verified intact" from "none present". **It only covers the manifest → Salesforce leg.** The baseline is `output/pushed_<UTC>.csv`, written *after* the ThoughtSpot fetch, so if `/searchdata` ever began returning folded text the manifest and Salesforce would agree and it would pass forever. Nothing now covers that leg — a recurrence needs a fresh comparison of the API response against the ThoughtSpot UI for a named member.

  **Never normalize the values you fetch or write** — no transliteration map, no accent stripping, no NFKC on the payload. (Normalizing a *comparison*, as `verify_push.py` does, is a different thing and is fine: it decides whether two values are the same text, it does not change what gets stored.) The sync is reused by chapters in other countries, so every character check must be `ord(c) > 127`, never a per-language letter list. If accents ever *do* start disappearing, note that `Ł` (U+0141) has no Unicode decomposition, so NFKD leaves it alone while `Ą` NFKD-folds to `A`: seeing `Ł`→`L` means a real transliteration table (Unidecode, `iconv //TRANSLIT`, ICU, a cleansing package), not naive normalization.

- **`output/members.csv` is NOT the record of what a push sent.** `fetch_active()` writes it *before* `--limit` / `--personids` filtering and before the push runs at all, and any later `fetch_members.py` run overwrites it. The push writes its own timestamped `output/pushed_<UTC>.csv` manifest (`reporting.write_push_manifest`, called before the write so it survives a crash) — that is what `verify_push.py` compares against. Don't point a verifier at `members.csv`: after a partial push it names records that were never submitted, and every one would be reported as a mismatch.

- **No checker can detect a *blanking* regression.** `verify_push.py` skips fields the source would omit, so if the write ever started sending nulls again it would report a clean pass. It holds no before-snapshot, and nothing else looks at omitted fields now. The only real guard is the omit rule in `salesforce_client.upsert_contacts` itself; treat it as load-bearing and don't relax it on the strength of a green verification run.
...
- **An external-ID upsert can fail to match a record that exists.** This cost a production run: 209 of 2,463 came back `DUPLICATE_VALUE ... duplicates value on record with id: <the record it should have updated>`. Salesforce's lookup runs under the integration user's sharing rules, but the unique index is org-wide — so a Contact that user can't see is simultaneously invisible (→ insert) and present (→ index rejects it). Soft-deleted records in the Recycle Bin do the same thing, and are **not** fixed by resolving ids ourselves — they stay invisible to the query while still holding the index, so the insert still collides. Those need restoring or hard-purging. Resolving ids ourselves makes the match inspectable, but **does not fix the visibility case** — that needs View All *and* Modify All on Contact (View All alone makes them resolve, then the PATCH fails `INSUFFICIENT_ACCESS_OR_READONLY`). Telling the cases apart needs a `/queryAll` lookup of the failing ids (soft-deleted rows are invisible to a normal query but still hold the index) plus a check of what the integration user can see; the script that did it is in git history — see *Retired tooling*.
...
- **Sentinel dates are `-2208988800` (`1900-01-01`), and they are NOT "no expiry".** Measured 2026-09-18 over 10,495 live term rows: it is the *only* out-of-range value present, on **9 rows**, and those rows carry it in `Pmpstartdate`, `Pmpexpiredate` **and** `Pmporiginalgrantdate` simultaneously. A credential that never expires would still have a grant date, so "no expiry" is ruled out.

  **A different placeholder would be WRITTEN, not omitted.** `-2208988800` is caught only because it falls outside `[_MIN_EPOCH, _MAX_EPOCH]` = `[0, 7258118400]`. If the source ever switched to `0` (1970-01-01) or any other in-range "no date" marker, `is_plausible_date_epoch()` would accept it and `to_salesforce_date()` would write it as a real date — and nothing would flag it, since `verify_push.py` only scans for the literal `1900-01-01`. If the source changes shape, check for implausibly early in-range dates (anything before ~1990) as well as the known sentinel.

  **What it does mean is not yet settled.** The best-supported reading is *"no dates recorded"* — those rows have a non-empty `Pmppipelinestatus` and `Certificationlist`. But the report only tested those two fields for *non-emptiness*: it never checked whether the status value means the person actually holds a PMP (a *pipeline* status may well be "In Progress"), nor whether `Certificationlist` contains `PMP` rather than only CAPM/ACP. And 9 rows at (person, term) grain could be as few as one person. Treat "no dates recorded" as a hypothesis, not a fact: settling it needs the distinct status values, whether `Certificationlist` actually contains `PMP`, and the distinct person count behind those 9 rows — which is exactly what `report_sentinel_dates.py` §3 printed; restore it from git history rather than rewriting. Either way `to_salesforce_date()` maps it to `None` and the write omits it — never write a placeholder date.
...
Eight one-off scripts were deleted on 2026-09-19 once the questions they
answered were settled and recorded in the gotchas above. **They are not gone —
`git log --diff-filter=D --name-only` finds the commit, and
`git show <commit>^:<file>` restores one.** Recover rather than rewrite if the
matching problem recurs:

| Script | Answered | Where the answer lives now |
|---|---|---|
| `investigate_fields.py`, `investigate_currency.py` | What `Isactive` really means | The `Isactive` gotcha. Note it *also* asked two things never answered: whether the `Membership*forchapter` date columns are person-stable enough to replace manual dedupe, and whether `Isactive=true` includes people with no chapter (`Ishaschapter`) |
| `investigate_certifications.py`, `check_cert_data.py` | How certs are represented at person grain | `FIELD_MAP` + the cert fields themselves |
| `test_upsert_update.py` | That a push matches existing Contacts rather than duplicating them — by re-querying and counting Contacts per PMI id | `verify_push.py`'s missing/ambiguous checks, over the whole pushed population. **⚠ The only retired script that WROTE to Salesforce** — it stamped `_test` onto `FirstName` and `2000-01-01` into `Chapter_Join_Date__c`. Don't restore it against production without reading it first |
| `diagnose_duplicates.py` | Why 209 Contacts failed `DUPLICATE_VALUE` | The external-ID gotcha, plus NEXT_STEPS §13b for what the triage established. Fixed by **View All + Modify All** |
| `report_sentinel_dates.py` | Whether omit-empties leaves stale data (§4), and what the sentinel co-occurs with (§3) | The omit-empties and sentinel gotchas — but **§3 is still the instrument for the open "what does the sentinel mean" question**, and §4 for re-measuring before relaxing the omit rule. Restore rather than rewrite |
| `diagnose_diacritics.py` | Where accented characters were lost | The accents gotcha — nowhere, they survive |

What remains is the pipeline plus five standing scripts: `verify_push.py` (after
every push), `test_sf_auth.py` (before a first push from a new environment), and
`list_datasets.py` / `list_columns.py` / `debug_searchdata.py` (when the source
changes shape).
